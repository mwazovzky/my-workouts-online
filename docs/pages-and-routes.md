...
| `WorkoutLogEdit`      | `/workout-logs/{id}/edit`                                                                                  | Yes        | Edit an in-progress log — manage activities and sets locally, save all changes at once (owner-scoped) |
...
| Save workout (bulk)         | PATCH  | `/workout-logs/{workoutLog}/save`     | `workout.logs.save`     | `WorkoutLogPolicy@save` — owner + status must be `in_progress`     |
...
- **Policy-guarded mutations**: `WorkoutLogPolicy` checks `user_id` match and enforces status prerequisites.
